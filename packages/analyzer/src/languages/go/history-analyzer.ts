...
export type AnalyzeGoHistoryInput = AnalyzeRepositoryHistoryInput;
export type AnalyzeGoHistoryOutput = AnalyzeRepositoryHistoryOutput;

export async function analyzeGoHistory(
  input: AnalyzeGoHistoryInput,
): Promise<AnalyzeGoHistoryOutput> {
  if (!input.detectedKinds.includes("go")) {
    throw new Error("repository is not detected as go");
  }
  ...
  const rawResults: GoSnapshotResult[] = Array.from({ length: sampledCommits.length });

  await runWithConcurrency(
    sampledCommits.map((_, snapshotIndex) => async () => {
      rawResults[snapshotIndex] = await analyzeGoSnapshot({
        input,
        publish,
        startedAtMs,
        totalCommits: commits.length,
...
async function analyzeGoSnapshot(input: {
  input: AnalyzeGoHistoryInput;
  ...
  for (const bucketCommit of candleBoundaryCommits) {
    throwIfAborted(input.input.abortSignal);
    const bucketModules = (await detectGoModulesAtRevision(input.input.localPath, bucketCommit.hash)).filter(
  ...
  if (previousCommit !== null) {
    ...
    for (const diffRow of diffRows) {
      ...
      if (
        attributionPath &&
        isGoSourcePath(attributionPath) &&
        attributedModule &&
        !diffRow.isBinary &&
        diffRow.added !== null &&
  ...
  return {
    ...
    moduleMetaByKey: new Map(
      modules.map((module) => [module.key, { name: module.name, kind: module.kind }]),
    ),
  };
}
...
function isGoSourcePath(filePath: string): boolean {
